...
import { wbr, thinSpace, applyWrapperStyle, applyLatinClass, applyNoBreakStyle } from '../src/utils-tags'
...
describe('Typesetter', () => {
  ...
  const expectedHtml = `<p>${applyWrapperStyle(`${applyNoBreakStyle('──', prefix)}${space}「`, prefix, addwbr)}<b>${applyWrapperStyle(`こんにちは。`, prefix, addwbr)}</b>${applyWrapperStyle(`」${space}${wbr}日本語${wbr}と${space}${wbr}${applyLatinClass('English', prefix)}${space}、${space}${wbr}晴れ${space}・${space}${wbr}${applyLatinClass('28', prefix)}${space}${wbr}度。`, prefix, addwbr)}</p>`
